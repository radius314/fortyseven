...
# attempt to only find time that is not in a preformatted slack section (`something`)
...
module.exports = (robot) ->
  ...
  userTimeZoneForUser = (user) ->
    return unless user
    return if user.slack?.is_bot
    return unless user?.slack?.tz or user?.tz
    {
      tz: user.slack?.tz || user.tz,
      tz_label: user.slack?.tz_label || user.tz_label,
      tz_offset: user.slack?.tz_offset || user.tz_offset
    }
  ...
  roomTimeZonesForName = (name) ->
    channelTimeZonesForChannel robot.adapter.client.rtm.dataStore.getChannelGroupOrDMById name
  ...
  robot.hear HUBOT_TIMEZONE_CONVERTER_REGEX, (res) ->
    ...
    if messageDate
      ...
      if roomTimeZones.length > 1 or res.message.room is res.message.user.name
        memberDates = roomTimeZones.map((roomTimeZone) ->
          moment.tz(messageDate, roomTimeZone.tz)
            ...
            # shorten some common timezone names
            .replace(/Eastern Standard Time/i, 'EST')
            .replace(/Eastern Daylight Time/i, 'EDT')
            .replace(/ (?:Standard )?Time/i, '')
          )
          .reduce((zones, zone) ->
            # group timezones with same offset
            zoneNameMatch = /\((.+)\)/.exec(zone)
            if zoneNameMatch
              zoneName = zoneNameMatch[1]
              index = zones.findIndex((z) -> z.slice(0, 6) == zone.slice(0, 6))
              if index >= 0
                existentZone = zones[index]
                zones[index] = existentZone.slice(0, existentZone.lastIndexOf(')')) +
                  "/#{zoneName}), "
                return zones
            zones.push(zone)
            zones
        ...
        # everyone is on the same time (even if multiple timezones)
        return if memberDates.length < 2
